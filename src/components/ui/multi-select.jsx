...
const MultiSelect = ({
  ...
  const handleOptionClick = (option) => {
    ...
    } else if (!maxCount || selectedOptions.length < maxCount) {
      // Add the option if not selected and within limit
  ...
  const renderSelectedOptions = () => {
    ...
    if (selectedOptions.length > maxDisplayCount) {
      const displayOptions = selectedOptions.slice(0, maxDisplayCount);
      const remainingCount = selectedOptions.length - maxDisplayCount;
      return `${displayOptions.map((option) => option.name).join(", ")} +${remainingCount} more`;
    } else {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <div
        className="w-full cursor-pointer rounded-lg border border-gray-300 bg-white p-2 text-sm"
